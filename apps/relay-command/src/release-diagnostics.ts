...
async function executeExactTool(
  ...
  if (!context?.executeTool) {
    return { ok: false, error: "document.modelContext.executeTool is unavailable" };
  }
...
async function registerReleaseTools(): Promise<void> {
  ...
  await registerTool({
    ...
    title: "Export complete signed-transaction audit evidence",
    description: "Bind the exact deployed commit, final plan, matching human-approved PACT scopes, provider states and one receipt per final operation to a canonical SHA-256 digest. Read-only.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    annotations: { readOnlyHint: true, untrustedContentHint: false },
    execute: async () => {
      try {
        const [plan, mesh, releaseIdentity] = await Promise.all([
          readLocalTool("relay_get_plan"),
          readLocalTool("relay_get_mesh_state"),
          readLocalTool("relay_get_release_identity"),
        ]);
        if (!semanticSuccess(releaseIdentity)) {
          return toolOutput({
            ok: false,
            code: "AUDIT_RELEASE_IDENTITY_FAILED",
            releaseIdentity,
            message: "The final audit cannot certify a transaction whose compiled commit, trusted edge header and release manifest are inconsistent.",
          });
        }
        ...
        const consistency = await evaluateAuditConsistency(plan, approvals);
        const bundle = {
        ...
        return toolOutput({
          ...
          fallback: "Call relay_get_release_identity, relay_get_plan and relay_get_mesh_state directly and preserve their raw JSON outputs.",
        });
      }
    },
  });
